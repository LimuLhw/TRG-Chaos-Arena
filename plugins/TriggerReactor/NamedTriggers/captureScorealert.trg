 //스코어 발판 생성시 알림
IF captureFootCount == null && {?"capture.genScoreFoot"} == null
	captureFootCount = random(30, 33)
ENDIF
#LOG"[점령전] ScoreFoot Active , "+captureFootCount+" sec generate"
WHILE captureFootCount >= 0
	IF {?"capturebattle.inGame"} != true
		IF {?"CaptureDebug"} == true
			#log "[점령전] ScoreFoot active disabled"
		ENDIF
		#STOP
	ENDIF
	captureFootCount-=1
	IF captureFootCount < 0
		IF {?"CaptureDebug"} == true
			#log "[점령전] ScoreFoot active Time!"
			...
			SYNC
				...
				#LOG "[점령전] ScoreFoot actived to "+scorefoot
				IF scorefoot == 1
